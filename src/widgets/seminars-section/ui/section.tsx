...
export default function SeminarsSection() {
  const motionSVGRef = useRef<SVGGElement | null>(null)
  const motionPathRef = useRef<SVGPathElement | null>(null)
  const beeRef = useRef<SVGGElement | null>(null);

  useGSAP(() => {
    if (!motionPathRef.current) return

    gsap.set(motionSVGRef.current, { scale: 0.85, autoAlpha: 1 });
    gsap.set(beeRef.current, { transformOrigin: "50% 50%", scaleX: -1 });
    const getProp = gsap.getProperty(motionSVGRef.current)
    let flippedX = false,
      flippedY = false;
    gsap.to(motionSVGRef.current, {
      scrollTrigger: {
        ...
        onUpdate: self => {
          ...
          if (flipY !== flippedY || flipX !== flippedX) {
            gsap.to(beeRef.current, { scaleY: flipY ? -1 : 1, scaleX: flipX ? -1 : 1, duration: 0.25 });
            flippedY = flipY;
            flippedX = flipX;
          }
        }
      ...
      duration: 10,
      ease: pathEase(motionPathRef.current, { smooth: true }), // <-- MAGIC!
    ...
    */
    type PathInput = string | SVGPathElement;
    interface PathEaseConfig {
      smooth?: boolean | number
      precision?: number
      axis?: "x" | "y"
    }
    function pathEase(path: PathInput, config: PathEaseConfig = { smooth: true, axis: "y" }) {
      const axis = config.axis || "y",
        precision = config.precision || 1,
        rawPath = MotionPathPlugin.cacheRawPathMeasurements(MotionPathPlugin.getRawPath(gsap.utils.toArray<PathInput>(path)[0]), Math.round(precision * 12)),
        useX = axis === "x",
  ...
  return (
    <section>
      <div>
        ...
        <div className="flex max-w-7xl overflow-auto">
        ...
        <svg id="bee-scroll" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 1588.4 2762.3">
          <defs>
            <clipPath id="clip-path">
              <rect id="Rectangle_2885" data-name="Rectangle 2885" width="89.252" height="72.066" fill="none" />
            </clipPath>
          </defs>
          <path ref={motionPathRef} className="fill-none" d="M37.5,31C32.5,41.2,52.3,122.6,358,237.2c222.1,69.8,610.9-11.5,861.3,82.5
          ...
          <g ref={motionSVGRef} data-name="Group 1117">
            <g ref={beeRef}>
